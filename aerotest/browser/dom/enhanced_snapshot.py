...
提供无状态函数，用于解析 Chrome DevTools Protocol (CDP) DOMSnapshot 数据�?
提取可见性、可点击性、光标样式和其他布局信息�?
...
# 只包含交互性和可见性检测所必需的计算样�?
# 只获取代码中实际使用的样式（防止在大型网站上导致 Chrome 崩溃�?
REQUIRED_COMPUTED_STYLES = [
    'display',  # 用于 service.py 可见性检�?
    'visibility',  # 用于 service.py 可见性检�?
    'opacity',  # 用于 service.py 可见性检�?
    'overflow',  # 用于 views.py 可滚动性检�?
    'overflow-x',  # 用于 views.py 可滚动性检�?
    'overflow-y',  # 用于 views.py 可滚动性检�?
...
def _parse_rare_boolean_data(rare_data: dict, index: int) -> Optional[bool]:
    ...
    Args:
        rare_data: 罕见数据字典（包�?'index' 键）
        index: 要检查的索引
    
    Returns:
        如果索引在罕见数据中，返�?True，否则返�?None
    """
...
def build_snapshot_lookup(
    ...
    """
    构建后端节点 ID 到增强快照数据的查找�?
    
    这个函数预先计算所有内容，避免在后续处理中重复计算�?
    
    Args:
        snapshot: CDP DOMSnapshot.captureSnapshot 的返回�?
        device_pixel_ratio: 设备像素比（用于坐标转换�?
    
    Returns:
        backend_node_id -> EnhancedSnapshotNode 的映�?
    """
