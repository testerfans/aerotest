...
class BaseFunnelLayer(ABC):
    ...
    def log_start(self):
        """记录层开始处�?""
        self.logger.info(f"🔍 {self.layer_name} 开始处�?)
    
    def log_end(self, result_count: int = 0):
        """记录层处理完�?""
        self.logger.info(f"�?{self.layer_name} 处理完成，找�?{result_count} 个候�?)
...
class FunnelEngine:
    ...
    def add_layer(self, layer: BaseFunnelLayer):
        ...
        self.logger.debug(f"添加�? {layer.layer_name}")
