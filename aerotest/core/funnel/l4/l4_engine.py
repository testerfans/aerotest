...
class L4Engine(BaseFunnelLayer):
    ...
    Example:
        ...
        # 处理复杂逻辑
        context = FunnelContext(instruction="选择最便宜的商品")
        context.l2_candidates = [...]  # L2 的候选
    ...
    def __init__(
        ...
        Args:
            ...
            use_l4_for_ambiguity: 是否在模糊场景使用 L4
        """
        super().__init__("L4")
    ...
    async def process(
        ...
        if not context.action_slot:
            self.logger.warning("没有槽位信息，跳过 L4")
            return context
        ...
        if context.l2_candidates and len(context.l2_candidates) > 0:
            ...
            if best_score >= self.confidence_threshold:
                self.logger.info(f"L2 置信度足够高({best_score:.2f})，跳过 L4")
                return context
        
        if context.l3_candidates and len(context.l3_candidates) > 0:
            best_score = context.l3_candidates[0].score
            if best_score >= self.confidence_threshold:
                self.logger.info(f"L3 置信度足够高({best_score:.2f})，跳过 L4")
                return context
        
        # 2. 获取候选元素（优先 L3，其次 L2）
        ...
        self.logger.info(f"使用 L4 处理 {len(candidates)} 个候选")
        ...
        try:
            ...
            if "selected_index" in result:
                ...
                if 0 <= selected_index < len(candidates):
                    ...
                    self.logger.error(f"AI 返回的索引超出范围: {selected_index}")
            else:
